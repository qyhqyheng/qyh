...
public class DataBoardView extends View {
    ...
    final String TAG = "DataBoardView";
    ...
    private float mShowDataValue = 0.0f; // 处理过的实时值
    private float mDataValue = 0.0f; // 真实的实时值
    ...
    private int[] color = new int[8];//渐变颜色组

    public DataBoardView(Context context) {
        this(context, null);
    }

    public DataBoardView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public DataBoardView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mUnitTextSize = a.getDimensionPixelSize(R.styleable.NoiseboardView_unitTextSize, spToPx(36));
    ...
    private void init() {
        ...
        initAngle = getAngleFromResult(mShowDataValue);
        ...
        color[4] = mViewColor_red;
        color[5] = mViewColor_red;
        color[6] = mViewColor_red;
        color[7] = mViewColor_red;
    ...
    protected void onDraw(Canvas canvas) {
        // 绘制色带
        canvas.drawArc(mRectRibbon, 150, 239, false, mPaintRibbon);
        ...
        } /*else if (mShowDataValue > 40 && mShowDataValue <= 90) {
            mPaintValue.setColor(mViewColor_yellow);
            mPaintCirclePointer.setColor(mViewColor_yellow);
        } else if (mShowDataValue > 90 && mShowDataValue <= 120) {
        ...
        canvas.drawCircle(mCenterX, mCenterY, dpToPx(3), mPaintCirclePointer);
        ...
        //绘制实时值
        canvas.drawText(trimFloat(mDataValue), mCenterX, mCenterY + mRadius/2 , mPaintValue);
     ...
     * @param realTimeValue
     */
    public void setRealDataValue(float realTimeValue) {
/*
        if (realTimeValue > mMaxValue) return;
*/      mDataValue = realTimeValue;
        if (realTimeValue > mMaxValue){
            realTimeValue = 100;
        }
        mShowDataValue = realTimeValue;
        initAngle = getAngleFromResult(mShowDataValue);
        invalidate();
    }
