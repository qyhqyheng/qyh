...
public class DashboardView extends View {
    ...
    private boolean LimitFalg = false;
    private long Show_Frequence = 0;
    private int Show_Sum = 2;
    ...
    public DashboardView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mHeaderTextSize = a.getDimensionPixelSize(R.styleable.DashboardView_headerTextSize, spToPx(30));
        ...
        mCircleRadius = a.getDimensionPixelSize(R.styleable.DashboardView_circleRadius, mRadius / 20);
        mMinValue = a.getInteger(R.styleable.DashboardView_minValue, 0);
        mMaxValue = a.getInteger(R.styleable.DashboardView_maxValue, 20);
    ...
    private void drawStripe(Canvas canvas) {
        if (mStripeMode != StripeMode.NORMAL && mStripeHighlight != null) {
            for (int i = 0; i < mStripeHighlight.size(); i++) {
                ...
                BleLog.d("StartAngle()="+highlightCR.getStartAngle()+"---SweepAngle()="+highlightCR.getSweepAngle());
                mPaintStripe.setColor(highlightCR.getColor());
                if (highlightCR.getStartAngle() + highlightCR.getSweepAngle() <= mStartAngle + mSweepAngle) {
                    if(i==0){
                        canvas.drawArc(mRectStripe, highlightCR.getStartAngle()-10,
                                highlightCR.getSweepAngle()+10, false, mPaintStripe);
                    }else {
                    ...
                    if(i==0){
                        canvas.drawArc(mRectStripe, highlightCR.getStartAngle()-10,
                                mStartAngle + mSweepAngle - highlightCR.getStartAngle()+10, false, mPaintStripe);
                    }else {
                        canvas.drawArc(mRectStripe, highlightCR.getStartAngle(),
                                mStartAngle + mSweepAngle - highlightCR.getStartAngle()+10, false, mPaintStripe);
                    }
    ...
    private void drawMeasures(Canvas canvas) {
        ...
        for (int i = 0; i <= mBigSliceCount; i++) {
            ...
            BleLog.d("mBigSliceAngle="+mBigSliceAngle+"000---mStartAngle="+mStartAngle);
    ...
    private void drawCircleAndReadingText(Canvas canvas) {
        ...
        LimitFalg = (MediaManager.getInstance().AlarmValue>mRealTimeValue?true:false);
        BleLog.e("MediaAction------AlamValue="+MediaManager.getInstance().AlarmValue+"   LimitFalg="+LimitFalg);
        if(LimitFalg){
            if(mRealTimeValue==0){
                Show_Frequence++;
                if(Show_Frequence%Show_Sum==0){
                    return;
                }
